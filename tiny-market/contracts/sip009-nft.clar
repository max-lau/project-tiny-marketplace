...
(define-non-fungible-token my-lovely-nft uint)
...
(define-read-only (get-owner (token-id uint))
	(ok (nft-get-owner? my-lovely-nft token-id))
)

(define-public (transfer (token-id uint) (sender principal) (recipient principal))
	(begin
		(asserts! (is-eq tx-sender sender) err-not-token-owner)
		(nft-transfer? my-lovely-nft token-id sender recipient)
	)
)
...
(define-public (mint (recipient principal))
	(let ((token-id (+ (var-get token-id-nonce) u1)))
		...
		(try! (nft-mint? my-lovely-nft token-id recipient))
